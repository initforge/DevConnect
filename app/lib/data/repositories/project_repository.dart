...
class ProjectRepository {
  ...
  Future<List<Project>> getProjects({int limit = 50}) async {
    if (_useApi) {
      final data = await ApiService.instance.get('/api/projects', queryParams: {'limit': limit});
      final projects = data.map((json) => ModelMappers.projectFromJson(json as Map<String, dynamic>)).toList();
      await _saveProjectsToDb(projects);
      return projects;
    }
    final db = await _database.database;
    final rows = await db.query('projects', orderBy: 'created_at DESC', limit: limit);
  ...
  Future<List<Project>> searchProjects(String query) async {
    if (_useApi) {
      ...
      return data.map((json) => ModelMappers.projectFromJson(json as Map<String, dynamic>)).toList();
    }
    final db = await _database.database;
  ...
  Future<bool> joinProject(String projectId) async {
    if (_useApi) {
      final result = await ApiService.instance.post('/api/projects/$projectId/join', {});
  ...
  Future<void> _saveProjectsToDb(List<Project> projects) async {
    ...
    await db.transaction((txn) async {
      for (final project in projects) {
        await txn.insert('projects', ModelMappers.projectToRow(project),
